...
estoque = {
    ...
    3: {"nome": "Opções de Molhos", "quantidade": "10 Kg", "preco": 20.00, "tipo": ["Tradicional", "Picante", "Agridoce"], "borda": "Cheddar"},
    4: {"nome": "Tipo de Massa", "quantidade": "10 UNI", "preco": 25.00, "tipo": ["Tradicional", "Sem Glúten"], "borda": "Requeijão"},
    5: {"nome": "Calabresa", "quantidade": "7 Kg", "preco": 25.00, "tipo": ["Artesanal", "Defumada", "Pepperoni"], "borda": "Cream Cheese"},
    6: {"nome": "Ingredientes Extras", "quantidade": "5 Kg", "preco": 8.50, "tipo": ["Azeitona", "Milho", "Orégano", "Alho Frito"], "borda": "Sem Borda recheada"}
}
...
def carregar_estoque():
    if not os.path.exists(ARQUIVO_ESTOQUE):
        with open(ARQUIVO_ESTOQUE, "w", encoding="utf-8"): pass
        return
...
def adicionar_item():
    while True:
        ...
        while True:
            preco_str = input("Preço (ex: 12.50): ").strip().replace(",", ".")
            try:
...
def remover_item():
    while True:
        nome_remover = input("Digite o nome do ingrediente a remover: ").strip().title()
        encontrados = [id_item for id_item, dados in estoque.items() if dados["nome"] == nome_remover]
        
        if encontrados:
            for id_item in encontrados:
                del estoque[id_item]
            print(f"{nome_remover} removido com sucesso.")
            salvar_estoque()
        else:
            print(f"Ingrediente '{nome_remover}' não encontrado no estoque.")
...
def escolher_opcao_geral(nome_categoria, multiplos=False):
    for id_item, dados in estoque.items():
        if dados["nome"] == nome_categoria and "tipo" in dados:
            ...
            for i, opcao in enumerate(opcoes, 1):
                print(f"{i}. {opcao}")
            while True:
                escolha = input("Digite o número(s): ").strip()
                ...
                if multiplos:
                    ...
                    if len(numeros) == 1 and 1 <= int(numeros[0]) <= len(opcoes):
                        return opcoes[int(numeros[0]) - 1]
                print("Entrada inválida. Tente novamente.")
    return None if not multiplos else []
...
def fazer_pedido():
    print("=== FAZER PEDIDO ===")
    massa = escolher_opcao_geral("Tipo de Massa")
    molho = escolher_opcao_geral("Opções de Molhos")
    ingredientes_escolhidos = escolher_ingredientes_principais()
    extras = escolher_opcao_geral("Ingredientes Extras", multiplos=True)
    ...
    for id_item in ingredientes_escolhidos:
        ...
        if "tipo" in estoque[id_item]:
            tipo_escolhido = escolher_opcao_geral(nome, multiplos=True)
    ...
    print("\n" + "="*30)
    print("RESUMO DO PEDIDO".center(30))
    print("="*30)
